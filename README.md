# 👋 Hi, I'm Palak Gupta

🎓 B.Tech Data Science Student (3rd Year)
💻 Aspiring Software Engineer | Data Science Enthusiast
📍 India

---

## 🚀 About Me

* 📊 Passionate about **Data Science, Machine Learning & Backend Development**
* 💡 Love building real-world projects that solve problems
* 📈 Exploring **Deep Learning & MLOps**

---

## 🛠️ Tech Stack

### 👩‍💻 Languages

* Python 🐍
* JavaScript
* PostgreSQL
* SQL
* Java

### ⚙️ Tools & Technologies

* Pandas, NumPy, Scikit-learn
* Node.js, Express.js
* Git & GitHub
* MongoDB

---

## 📂 Projects

### 🔹 Personal Expense Tracker

* Built using Python
* Tracks daily expenses with dashboard
* Helps in financial analysis

### 🔹 Sales Prediction System

* Machine Learning model to forecast sales
* Used regression techniques
* Provided business insights

### 🔹 Car Price Prediction

* Predicts car prices using ML algorithms
* Data preprocessing + model evaluation

---

## 📚 Currently Learning

* Deep Learning
* PyTorch / TensorFlow
* System Design Basics
* Data Structures & Algorithms

---

## 🎯 Goals

* Get a **top tech internship (SDE/Data Science)**
* Contribute to **Open Source**

---

## 📫 Connect With Me

* 💼 LinkedIn: (https://www.linkedin.com/in/palak-gupta-572b8729a/)
* 📧 Email: (guptapalak1432@gmail.com)

---

## ⚡ Fun Fact

I turn ☕ coffee into 💻 code and 📊 insights

---

⭐ *Feel free to explore my repositories and give them a star!*
